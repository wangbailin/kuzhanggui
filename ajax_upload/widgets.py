...
class AjaxClearableFileInput(forms.ClearableFileInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        if value:
            ...
            logger.debug("%s name %s" % (str(type(value)), file_name))
            if file_name.startswith('baidu_yun'):
                filename = baidu_yun.url(file_name)
            else:
    ...
    def value_from_datadict(self, data, files, name):
        ...
        if file is not None:  # super class may return a file object, False, or None
            ...
            elif file_path.startswith("http://bcs.duapp.com"):
                seps = file_path.split('/')
                file_name = seps[-1]
                return baidu_yun.open(file_name)
            else:
