...
def upload(request):
    ...
    baidu_name = baidu_yun.ckeditor_upload(upload_filename, request.user.pk, THUMBNAIL_SIZE)
    url = baidu_yun.url(baidu_name)
    return HttpResponse("""
...
def get_image_browse_urls(user=None):
    ...
    """
    images = []
    for filename in baidu_yun.ckeditor_list(user.pk):
        images.append({
            'thumb': baidu_yun.ckeditor_thumb_url(filename),
            'src': baidu_yun.url(filename)
        })
